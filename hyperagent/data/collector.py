...
from hyperagent.data.batch import _alloc_by_keys_diff
from hyperagent.env import BaseVectorEnv, DummyVectorEnv
...
class Collector(object):
    ...
    :param policy: an instance of the :class:`~hyperagent.policy.BasePolicy` class.
    :param env: a ``gym.Env`` environment or an instance of the
        :class:`~hyperagent.env.BaseVectorEnv` class.
    :param buffer: an instance of the :class:`~hyperagent.data.ReplayBuffer` class.
    ...
    dict or a :class:`~hyperagent.data.Batch` with the modified keys and values. Examples
    ...
    def __init__(
        ...
        self._collect_regret = collect_regret
        self._total_regret = 0
        self._env_step = 0
        # avoid creating attribute outside __init__
        self.reset()

    def _target_noise(self):
        if self.target_noise_type in ["gbi", "sp"]:
            noise = np.random.randn(self.env_num, self.noise_per_sample, self.target_noise_dim).astype(np.float32)
            ...
            noise_shape = (self.env_num, self.noise_per_sample, self.target_noise_dim)
            noise = np.random.binomial(1, 0.5, size=noise_shape).astype(np.float32) / 0.5
        elif self.target_noise_type in ["gs"]:
            noise = np.random.randn(self.env_num, self.noise_per_sample, self.target_noise_dim).astype(np.float32)
        elif self.target_noise_type in ["exp"]:
            assert self.target_noise_dim % 2 == 0
            noise_1 = np.random.randn(self.env_num, self.noise_per_sample, self.target_noise_dim // 2).astype(np.float32)
            noise_1 /= np.linalg.norm(noise_1, axis=-1, keepdims=True)
            noise_2 = np.random.randn(self.env_num, self.noise_per_sample, self.target_noise_dim // 2).astype(np.float32)
            noise_2 /= np.linalg.norm(noise_2, axis=-1, keepdims=True)
            noise = np.concatenate([noise_1, noise_2], -1)
        elif self.target_noise_type in ["a-exp"]:
            noise = np.random.randn(self.env_num, self.noise_per_sample, self.target_noise_dim).astype(np.float32)
            noise /= np.sqrt(self.target_noise_dim // 2)
        elif self.target_noise_type in ["uni"]:
            if self.target_noise_index == self.target_noise_dim:
                self.target_noise_index = 0
                z = np.random.randn(self.target_noise_dim, self.target_noise_dim).astype(np.float32)
                q, r = np.linalg.qr(z)
                d = np.diag(r)
                ph = d / np.abs(d)
                self.noise = np.multiply(q, ph)
            noise = self.noise[:, self.target_noise_index]
            noise = noise.reshape(1, -1)
            self.target_noise_index += 1
        else:
    ...
    def _gen_ensemble_mask(self):
        ...
        if self._ensemble_mask == "bi":
            masks = np.random.binomial(1, self._ensemble_prob, size=mask_size)
        elif self._ensemble_mask == "exp":
            masks = np.random.exponential(self._ensemble_prob, size=mask_size)
        return masks
    ...
    def reset_env(self) -> None:
        ...
        if self.target_noise_dim:
            self.data.target_noise = self._target_noise()
    ...
    def collect(
        ...
        while True:
            ...
            if self._collect_regret:
                regret = info.tolist()[0].get('regret', 0)
                self._total_regret += regret
                self._env_step += 1
                regret_results = {
                    "regret": regret,
                    "acc_regret": self._total_regret,
                    "avg_regret": self._total_regret / self._env_step,
                    "env_step": self._env_step + 1,
                }
                if (self._env_step + 1) % 10 == 0 or self._env_step == 1:
                    logger.log_regret_data(regret_results, self._env_step + 1)

            self.data.update(obs_next=obs_next, rew=rew, done=done, info=info)
            if self._ensemble_num:
                self.data.update(ensemble_mask=self._gen_ensemble_mask())
            if self.target_noise_dim:
                self.data.update(target_noise=self._target_noise())
            if self.preprocess_fn:
                self.data.update(
                    self.preprocess_fn(
...
class AsyncCollector(Collector):
    ...
    The arguments are exactly the same as :class:`~hyperagent.data.Collector`, please
    refer to :class:`~hyperagent.data.Collector` for more detailed explanation.
    """
