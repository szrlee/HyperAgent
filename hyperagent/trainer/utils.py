...
def debug_episode(
    ...
    """A simple wrapper of testing policy in collector."""
    obs = env.reset()
    noise = policy.sample_noise((obs.shape[0], 40))
    logits, _ = policy.model(obs, noise=noise)
    q = policy.compute_q_value(logits, mask=None)[0]
    q = q.cpu().detach().numpy()
    result = {
        'debug/action_right_q_var': np.var(q[:, action_right]),
        'debug/action_left_q_var': np.var(q[:, 1 - action_right]),
        'debug/action_right_q_mean': np.mean(q[:, action_right]),
        'debug/action_left_q_mean': np.mean(q[:, 1 - action_right]),
    }
    if logger and global_step is not None:
        logger.log_debug_data(result, global_step)
    return result
