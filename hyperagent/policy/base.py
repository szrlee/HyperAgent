...
class BasePolicy(ABC, nn.Module):
    ...
    hyperagent aims to modularize RL algorithms. It comes into several classes of
    policies in hyperagent. All of the policy classes must inherit
    :class:`~hyperagent.policy.BasePolicy`.

    A policy class typically has the following parts:

    * :meth:`~hyperagent.policy.BasePolicy.__init__`: initialize the policy, including \
        coping the target network and so on;
    * :meth:`~hyperagent.policy.BasePolicy.forward`: compute action with given \
        observation;
    * :meth:`~hyperagent.policy.BasePolicy.process_fn`: pre-process data from the \
        replay buffer (this function can interact with replay buffer);
    * :meth:`~hyperagent.policy.BasePolicy.learn`: update policy with a given batch of \
        data.
    * :meth:`~hyperagent.policy.BasePolicy.post_process_fn`: update the replay buffer \
        from the learning process (e.g., prioritized replay buffer needs to update \
        the weight);
    * :meth:`~hyperagent.policy.BasePolicy.update`: the main interface for training, \
    ...
    Since :class:`~hyperagent.policy.BasePolicy` inherits ``torch.nn.Module``, you can
    use :class:`~hyperagent.policy.BasePolicy` almost the same as ``torch.nn.Module``,
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        :return: A :class:`~hyperagent.data.Batch` which MUST have the following keys:
    ...
    def map_action(self, act: Union[Batch, np.ndarray]) -> Union[Batch, np.ndarray]:
        ...
        This function is called in :meth:`~hyperagent.data.Collector.collect` and only
